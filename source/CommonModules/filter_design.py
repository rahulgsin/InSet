...
class gen_filter():
    ...
    def filter_design(self):
        ...
        self.freq = np.arange(0,Fs,Fs/len(self.ypt1))
        #self.b = np.array([self.const*self.parameters['Res']*self.parameters['Cap'],0.0])    #Numerator
        #self.a = np.array([self.parameters['Res']*self.parameters['Cap'],1.0])    #Denominator

        self.w = 2.0j*np.pi*self.freq 
        self.h = (self.const*self.w*self.parameters['Res']*self.parameters['Cap'])/(self.w*self.parameters['Res']*self.parameters['Cap']+1)
       ...
       # self.w, self.h = signal.freqs(self.b, self.a,worN=np.logspace(0, 10, 5000))
        ...
        plt.grid()
        plt.subplot(212)
        plt.plot(self.freq, ((180/np.pi)*np.angle(self.h)))
        plt.xscale('log')
        ...
        A = fft(self.ypt1) #/ (len(self.window)/2.0)  #fft of Input signal
        #freq = np.linspace(-10.0, 10.0, len(A))
        #response = np.abs(fftshift(A / abs(A).max()))
        plt.subplot(212)
        
        self.freq = np.arange(0,Fs,Fs/len(A))
        
        #self.tim2 = np.arange(0,10.0e6,10.0e3)
        plt.plot(self.freq,abs(A))
        #plt.xscale('log')
        ...
        #plt.subplot(212)
        plt.hold(True)
        plt.plot(self.xpt1,self.ypt1,self.xpt1,30*(N))
        #import ngplot
        plt.show()
