...
class gaussian():
    ...
    def gauss_fun(self,mean,sd):
        ...
        plt.figure(1)
        plt.plot(self.y,self.value) #plotting input function 
    ...
    def NGspice(self):
        ...
        call(["gnome-terminal","--command=ngspice bpm_gauss.cir"])    
        ...
        self.line1 = [line.split() for line in open('output.data',"r")]
        self.xpt = 0*np.ones(len(self.lines))
        self.ypt = 0*np.ones(len(self.lines))
        for i in range(len(self.lines)):
            self.xpt[i] = float(self.line1[i][0])
            self.ypt[i] = float(self.line1[i][1])
        plt.plot(self.xpt,self.ypt)
